...
export default function TextInput({ name, label, error, register, ...props }: TextInputProps) {
  ...
  return (
    <>
      ...
      <input id={name} {...(register ? register(name) : {})} className={`shadow-sm border text-sm rounded-md focus:outline-0 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${error ? 'border-red-400' : 'border-gray-300'}`} {...props} />
